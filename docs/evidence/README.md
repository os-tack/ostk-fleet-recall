...
The current live release runs immutable image tag `git-efe6fbf4e2f1` at source
commit `efe6fbf4e2f1c5b9daab2c5f4f65ebf38a49770f`, with the serving,
migration, seed, and reference-agent task-definition families all at revision
7. Its rich-seed task exited zero and upserted exactly 548 rows: 342
documentation chunks, 2 code chunks, and 204 operations chunks. A seven-query
public smoke check verified exact specification/code and migration conflict
mappings, relevant conflict-free CockroachDB, Rust, project-purpose, and
datastore-library results, and zero results/zero conflicts for nonsense. The
live UI was also browser-verified. GitHub Actions run
...
revision-7 write chain or replacement proof. The accompanying
...
reference-agent and replacement proofs were not rerun during the revision-7
cutover:
...
receipt boundary, not the current revision-7 deployment.
...
historical proof. It was not rerun during the revision-7 cutover. Its SHA-256 is
