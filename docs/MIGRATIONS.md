...
Fleet Recall currently has one embedded schema migration,
`migrations/0001_fleet_memory.sql`. It creates the distributed corpus, claim and
conflict ledgers, audit tables, vector indexes, and lexical inverted index.
...
2. Create separate migrator and runtime SQL principals. Store their TLS URLs as
   raw values in separate AWS Secrets Manager secrets.
...
7. Run the application's `health` command with the runtime credential. Health
   must report schema version 1, vector and lexical indexes, cosine support, and
...
- `SELECT`, `INSERT`, `UPDATE`, and `DELETE` on Fleet Recall tables;
...
For example, after connecting as an authorized administrator and substituting
the actual database/role names:

```sql
GRANT CONNECT ON DATABASE fleet_recall TO fleet_runtime;
GRANT USAGE ON SCHEMA public TO fleet_runtime;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO fleet_runtime;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO fleet_runtime;
```

Review the expanded grants with `SHOW GRANTS` and revoke unnecessary defaults.
