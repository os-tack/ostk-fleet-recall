...
fn generation_switch_succeeds_against_its_exact_expected_prior_pointer() {
    ...
    let admitted = AdmittedGenerationSwitchV1::from_test_witness(proposal, &generation_1).unwrap();
...
mod fixture_pinning {
    ...
    fn negative_fixtures_reject_an_unrecognized_field_at_decode_time() {
        ...
        assert!(decode_strict::<ChunkOccurrencePreimageV1>(record(NEGATIVE_LINE_FIXTURE)).is_err());
    }
    ...
    fn negative_fixture_rejects_a_manifest_retired_by_a_zero_successor() {
        let decoded: ManifestSupersessionV1 =
            decode_typed_canonical(record(NEGATIVE_ZERO_SUCCESSOR_SUPERSESSION_FIXTURE)).unwrap();
        assert_eq!(decoded.predecessor_manifest_id.to_string(), MANIFEST_ID);
    ...
    fn negative_fixtures_decode_structurally_but_fail_span_validation() {
        ...
        for bytes in [
            ...
            let decoded: ChunkOccurrencePreimageV1 = decode_typed_canonical(record(bytes)).unwrap();
            assert!(decoded.validate().is_err());
