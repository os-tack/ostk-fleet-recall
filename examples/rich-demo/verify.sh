...
if [ "$line_count" -ne 3015 ]; then
    fail "record count must be exactly 3015 (found $line_count)"
fi
...
if ! jq -s -e \
    ...
    | ($expected_doc_sources | length) == 30
    and ($expected_repository_sources | length) == 202
    and
    ([.[] | select(.source_config_id == "rich-demo:docs:v1")] | length) == 691
    and ([.[] | select(.source_config_id == "rich-demo:self-audit:v1")] | length) == 2
    and ([.[] | select(.source_config_id == "rich-demo:repository:v1")] | length) == 2118
