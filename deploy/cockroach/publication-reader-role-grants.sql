-- Long-lived publication-reader role boundary for the dedicated fleet_recall
-- database.
--
...
-- creates their roles.
--
-- The exact application principal fleet_publication, including its password or
...
-- grant, ownership, or future default held there by fleet_publication_reader or
-- fleet_publication, and separately inventory current and future inherited
...
BEGIN
    IF pg_catalog.current_database() <> 'fleet_recall' THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'publication reader policy must run in fleet_recall';
    END IF;
END
$$;
...
DO $$
DECLARE
    publication_schema_ready BOOL;
BEGIN
    SELECT count(*) = 17
       AND min(version) = 1
       AND max(version) = 17
       AND COALESCE(bool_and(success), false)
    INTO publication_schema_ready
    FROM public._sqlx_migrations
    WHERE version BETWEEN 1 AND 17;

    IF publication_schema_ready IS DISTINCT FROM true THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'publication reader role requires the complete successful migration prefix through 17';
    END IF;
END
$$;
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader policy requires exact quiesced principal fleet_publication with options {NOLOGIN}: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_gate
FROM [SHOW USERS]
...
BEGIN
    ...
    IF forbidden_application_schema THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'publication reader policy requires public to be the only application schema';
    END IF;
END
$$;
...
WITH public_default AS (
    ...
    WHERE public_default.object_type IN (
      ...
      AND (
          public_default.role = 'fleet_publication_reader'
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader policy permits only intrinsic PUBLIC type USAGE/all-roles routine EXECUTE, target PUBLIC routine EXECUTE, and target self-owner ALL future defaults: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_public_default_gate
...
WITH principal_default AS (
    ...
    WHERE principal_default.object_type IN (
      ...
      AND (
          principal_default.role = 'fleet_publication'
          AND NOT principal_default.for_all_roles
          AND principal_default.grantee = 'fleet_publication'
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication principal has non-intrinsic future-default authority: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_default_gate
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader policy requires PUBLIC to have no system privileges: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_public_system_gate
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication principal must have no system privileges: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_system_gate
FROM [SHOW SYSTEM GRANTS]
...
DO $$
DECLARE
    publication_principal_owns_object BOOL;
BEGIN
    SELECT EXISTS (
        SELECT 1
        ...
        WHERE owner_role.rolname = 'fleet_publication'
    ) INTO publication_principal_owns_object;

    IF publication_principal_owns_object THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'publication principal must not own database, schema, relation, function, or type objects';
    END IF;
END
$$;
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader policy found an unsafe PUBLIC grant before target creation: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_public_grant_gate
FROM forbidden_public_grant;

-- The fixed principal receives authority only through the logical reader role.
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication principal must have no direct object grants: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_grant_gate
FROM [SHOW GRANTS FOR fleet_publication]
WHERE grantee = 'fleet_publication';

-- Before creation or reapplication, neither role may participate in any role
-- graph except the already-installed exact non-admin reader-to-principal edge.
...
WITH forbidden_incident_edge AS (
    ...
    WHERE (
        edge.role_name IN ('fleet_publication_reader', 'fleet_publication')
        OR edge.member IN ('fleet_publication_reader', 'fleet_publication')
    )
      AND NOT (
          edge.role_name = 'fleet_publication_reader'
          AND edge.member = 'fleet_publication'
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader/principal role graph is not an exact leaf edge: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_edge_gate
...
WITH publication_default AS (
    ...
    FROM [SHOW DEFAULT PRIVILEGES FOR GRANTEE fleet_publication_reader
          IN SCHEMA public]
), forbidden_publication_default AS (
    SELECT 1
    FROM publication_default
    WHERE publication_default.object_type IN (
            'schemas', 'routines', 'tables', 'sequences', 'types'
    )
      AND (
          publication_default.role = 'fleet_publication_reader'
          AND NOT publication_default.for_all_roles
          AND publication_default.grantee = 'fleet_publication_reader'
          AND publication_default.privilege_type = 'ALL'
          AND publication_default.is_grantable
      ) IS NOT TRUE
)
SELECT IF(
    count(*) = 0,
...
) AS publication_reader_target_default_gate
FROM forbidden_publication_default;
...
DO $$
DECLARE
    publication_reader_owns_object BOOL;
BEGIN
    SELECT EXISTS (
        SELECT 1
        FROM pg_catalog.pg_database AS database_object
        ...
        JOIN pg_catalog.pg_roles AS owner_role
          ON owner_role.oid = type_object.typowner
        WHERE owner_role.rolname = 'fleet_publication_reader'
    ) INTO publication_reader_owns_object;

    IF publication_reader_owns_object THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            MESSAGE = 'publication reader role must not own database, schema, relation, function, or type objects';
    END IF;
END
$$;
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader policy found a grant outside the repairable fleet_recall.public boundary: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_out_of_boundary_grant_gate
...
) AS publication_reader_unexpected_role_edge_gate
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader role has a forbidden validity or provisioned-identity option: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_identity_option_gate
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader role options differ from exact NOLOGIN: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_exact_role_option_postcondition
FROM [SHOW USERS]
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader role inheritance postcondition failed: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_role_edge_postcondition
FROM forbidden_role_edge;

-- Reassert the dedicated-database PUBLIC boundary, remove DDL authority, and
-- reset every direct current-object privilege and grant option on the reader.
...
-- Exact direct public startup/status/chunk-recall/claim-recall surface. The
-- reader receives no sequence, DML, DDL, receipt/event, control, registry, or
-- reconciliation-only authority.
...
-- and eight non-grantable table SELECT rows. Because SHOW GRANTS FOR also
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader direct-grant postcondition differs from exact ten-row matrix: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_exact_grant_postcondition
FROM [SHOW GRANTS FOR fleet_publication_reader]
WHERE grantee = 'fleet_publication_reader';
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication principal direct-grant postcondition is nonzero: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_grant_postcondition
FROM [SHOW GRANTS FOR fleet_publication]
WHERE grantee = 'fleet_publication';
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader/PUBLIC/principal system-grant postcondition is nonzero: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_system_postcondition
FROM [SHOW SYSTEM GRANTS]
WHERE grantee IN (
    'public', 'fleet_publication_reader', 'fleet_publication'
);

SELECT IF(
    count(*) = 2
        AND COALESCE(bool_and(
            (username = 'fleet_publication_reader'
                AND options::STRING = '{NOLOGIN}')
            OR (username = 'fleet_publication'
                AND options::STRING = '{NOLOGIN}')
        ), false),
    1:::INT8,
    CAST(
        concat(
            'publication reader/principal option postcondition differs from exact NOLOGIN: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_option_postcondition
FROM [SHOW USERS]
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader/principal edge postcondition differs from exact non-admin leaf edge: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_principal_edge_postcondition
...
SELECT IF(
    ...
    CAST(
        concat(
            'publication reader final PUBLIC application boundary is nonzero: observed=',
            count(*)::STRING
        )
        AS INT8
    )
) AS publication_reader_public_grant_postcondition
